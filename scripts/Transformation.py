...
class ImageTransformation:
    ...
    def showOrigine(self):
        ...
        if self.save:
            ...
            os.rename(self.outDir+"/input_image.png", self.outDir+"/"+self.filename+"Origine.png")
    
    def showGaussian(self):
        pcv.params.device=0
        pcv.gaussian_blur(self.b_thresh, ksize = (31,31))
        if self.save:
            os.rename(self.outDir+"/0_gaussian_blur.png", self.outDir+"/"+self.filename+"Gaussian.png")
    
    def showMask(self):
        pcv.params.device=0
        pcv.apply_mask(img=self.img, mask=self.b_thresh, mask_color='white')
        if self.save:
            os.rename(self.outDir+"/0_masked.png", self.outDir+"/"+self.filename+"Mask.png")
    ...
    def showRoi(self):
        ...
        if self.save:
            os.rename(self.outDir+"/0_roi.png", self.outDir+"/"+self.filename+"Roi.png")    

    def showAnalyse(self):
        pcv.params.device=0
        pcv.analyze.size(img=self.img, labeled_mask=self.b_thresh, n_labels=1)
        if self.save:
            os.rename(self.outDir+"/1_shapes.png", self.outDir+"/"+self.filename+"Analyse.png")
    
    def showPseudolandmarks(self):
        pcv.params.device=0
        pcv.homology.x_axis_pseudolandmarks(img=self.img, mask=self.b_thresh, label="test")
        if self.save:
            os.rename(self.outDir+"/0_x_axis_pseudolandmarks.png", self.outDir+"/"+self.filename+"Pseudolandmarks.png")
    
    def showColorAnalyse(self):
        pcv.params.device=0
        _, hist_data = pcv.visualize.histogram(img=self.img, mask=self.b_thresh, hist_data=True)
        if self.save:
            os.rename(self.outDir+"/3_hist.png", self.outDir+"/"+self.filename+"ColorAnalyse.png")
        else:
            plt.plot(hist_data[hist_data['color channel']=='red']['pixel intensity'],hist_data[hist_data['color channel']=='red']['proportion of pixels (%)'],label = "red", color="red")
            plt.plot(hist_data[hist_data['color channel']=='blue']['pixel intensity'],hist_data[hist_data['color channel']=='blue']['proportion of pixels (%)'],label = "blue", color="blue")
            plt.plot(hist_data[hist_data['color channel']=='green']['pixel intensity'],hist_data[hist_data['color channel']=='green']['proportion of pixels (%)'],label = "green", color="green")
...
def showTransformation(tr, origine, gaussian,mask,roi,analyze, pseudolandmarks, color):
    if origine:
        tr.showOrigine()
    if gaussian:
        tr.showGaussian()
...
@click.command()
@click.option('-src',type=click.Path(exists=True, file_okay=False, writable=True), default=None, help='Folder source path')
...
@click.argument('directsrc', type=click.Path(exists=True, dir_okay=False, writable=True), required=False)
def main(src,dst, origine,mask, gaussian, roi, analyze,pseudolandmarks, color, directsrc=None):
    if((origine or mask or gaussian or roi or analyze or pseudolandmarks or color) == False):
        origine, mask, gaussian, roi, analyze, pseudolandmarks, color = True, True, True, True, True, True, True
    try :
        if directsrc:
            ...
            for pathRoot, folders, files in os.walk(src):
                print(pathRoot)
                pathRootDst = os.path.join(dst,re.sub(r"^"+ re.escape(src),"",pathRoot))
                if len(folders)>0:
